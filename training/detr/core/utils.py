...
from .infer import ImagePrediction, Prediction
...
def prepare_for_coco_detection(predictions: List[ImagePrediction]) -> List[Dict[str, Union[int, List[float], float]]]:
...
def move_to_device(batch: Union[Dict, List, Tuple, BatchFeature, torch.Tensor], device: torch.device) -> Union[Dict, List, Tuple, BatchFeature, torch.Tensor]:
    ...
    if isinstance(batch, dict):
        ...
        return BatchFeature({key: move_to_device(value, device) for key, value in batch.items()})
...
def convert_bbox_format(bbox: Tuple[float, float, float, float]) -> Tuple[float, float, float, float]:
    """Convert bounding box format from (xmin, ymin, xmax, ymax) to (xmin, ymin, width, height)."""
    xmin, ymin, xmax, ymax = bbox
    return xmin, ymin, xmax - xmin, ymax - ymin


def denormalize_bboxes(normalized_bboxes: List[List[float]], image_width: int, image_height: int) -> List[List[float]]:
...
def add_missing_image_ids(predictions: pd.DataFrame, all_ids: List[str]) -> pd.DataFrame:
...
def initialize_optimizer(model, config, n_steps) -> Tuple[torch.nn.Module, torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    ...
    if config['ema']['enabled']:
        ema = EMA(model, beta=config['ema']['beta'], update_after_step=config['ema']['update_after_step'], update_every=config['ema']['update_every'])
...
def get_train_dataloader(config, data_dir, processor):
    ...
    train_collator = Collator(train_augment_and_transform((processor.size['height'], processor.size['width']), config['transforms']['train']), processor)
    train_dataset = MalariaDataset(train_df, os.path.join(train_path, 'images'))
    train_dataloader = DataLoader(train_dataset, batch_size=config['training']['train_batch_size'], shuffle=True, num_workers=4, collate_fn=train_collator)
