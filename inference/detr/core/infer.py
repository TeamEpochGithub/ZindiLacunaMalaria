...
def infer(cfg, model: torch.nn.Module, data_loader: torch.utils.data.DataLoader, device: torch.device, processor: torch.nn.Module = None) -> tuple:
    """
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Inferring"):
            ...
            labels = [{k: v.to(device) for k, v in t.items()} for t in batch["labels"]] if "labels" in batch else None
            target_sizes = torch.tensor([Image.open(os.path.join(cfg.data.image_dir, img_name)).size[::-1] for img_name in img_names])
